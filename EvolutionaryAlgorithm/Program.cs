...
namespace EvolutionaryAlgorithm
{
    public static class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            {
                i => RunBenchmark(i, heuristic: Repair, fitness: OneMax, mu: 1, lambda: 2, stepSize: 1000,
                    mutationRateString: "sqrt(log(n))",
                    mutationRateFunc: n => Math.Max(2, (int) Math.Sqrt(Math.Log(n))),
                    repairChanceString: "1/(p*n)",
                    repairChanceFunc: n => (int) Math.Max(1, n / (Math.Max(2D, Math.Sqrt(Math.Log(n))) / n)),
                    learningRate: 2
                ),
                i => RunBenchmark(i, heuristic: Repair, fitness: OneMax, mu: 1, lambda: 2, stepSize: 1000,
                    mutationRateString: "sqrt(k/n)",
                    mutationRateFunc: n => (int) Math.Max(1, Math.Sqrt(n)),
                    repairChanceString: "sqrt(k/n)",
                    repairChanceFunc: n => (int) Math.Max(1, Math.Sqrt(n)),
                    learningRate: 2
                ),
                ...
                i => RunBenchmark(i, heuristic: Repair, fitness: Jump, mu: 1, lambda: 2, stepSize: 1000,
                    jump: 2,
                    mutationRateString: "sqrt(log(n))",
                    mutationRateFunc: n => Math.Max(2, (int) Math.Sqrt(Math.Log(n))),
                    repairChanceString: "1/(p*n)",
                ...
                i => RunBenchmark(i, heuristic: Repair, fitness: Jump, mu: 1, stepSize: 1000,
                    jump: 2,
                    lambdaString: "sqrt(n/k)",
                    lambdaFunc: n => (int) Math.Max(2, Math.Sqrt(n / 2D)),
                    mutationRateString: "sqrt(k/n)",
                    mutationRateFunc: n => (int) Math.Max(1, Math.Sqrt(n) * 2),
                    repairChanceString: "sqrt(k/n)",
                    repairChanceFunc: n => (int) Math.Max(1, Math.Sqrt(n) * 2),
                    learningRate: 1
                ),
                i => RunBenchmark(i, heuristic: Repair, fitness: Jump, mu: 1, stepSize: 1000,
                    jump: 4,
                    ...
                    mutationRateString: "sqrt(k/n)",
                    mutationRateFunc: n => (int) Math.Max(1, Math.Sqrt(n) * 4),
                    repairChanceString: "sqrt(k/n)",
                    repairChanceFunc: n => (int) Math.Max(1, Math.Sqrt(n) * 4),
                    learningRate: 1
                ),
